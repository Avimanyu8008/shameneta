...
export default function ComparePage() {
  ...
  const filterCandidates = (search: string) => {
    if (!search) return allCandidates.slice(0, 10);
  ...
  const CandidatePanel = ({
    ...
    const filtered = filterCandidates(search);
  ...
  const CandidateCompareCard = ({ data, onClear }: { data: { candidate: any; cases: any[] }; onClear: () => void }) => {
    ...
    <div>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 14, lineHeight: 1.7, maxWidth: 600 }}>
          Compare any two candidates' declared criminal charges and Public Risk Scores head-to-head.
        </p>
      </div>
